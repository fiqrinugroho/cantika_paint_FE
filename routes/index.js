    ...
    let branch = req.cookies.branch
      ...
      if(branchId == 0){
        ...
        await axios.get(`${host}/api/transaction/search`, {
          ...
          // Menangani respons
        res.render('transaction', { transaction: response.data.data, tgl: date, branch, branchId});

        })
        .catch(error => {
          // Menangani kesalahan
        res.render('transaction', { transaction:"", tgl: date, branch, branchId});
        });
      }else {
        await axios.get(`${host}/api/transaction/search`, {
          headers: {
  ...
  router.post('/penjualan/add', auth,(req, res) => {
    ...
    Promise.all(requests)
      .then(([response1, response2]) => {
        ...
        res.render('addTransaction', {transaction: transactionData, colors: itemsData, tgl: date, branch, branchId, token});
    ...
    let branch = req.cookies.branch
      ...
      if(branchId == 0){
        ...
        await axios.get(`${host}/api/shipment/search`, {
          ...
          // Menangani respons
        res.render('shipment', { shipment: response.data.data, tgl: date, branch, branchId});

        })
        .catch(error => {
          // Menangani kesalahan
        res.render('shipment', { shipment:"", tgl: date, branch, branchId});
        });
      }else {
        await axios.get(`${host}/api/shipment/search`, {
          headers: {
  ...
  router.post('/pengiriman/add', auth,(req, res) => {
    ...
    Promise.all(requests)
      .then(([response1, response2]) => {
        ...
        const itemsData = response2.data.data
        // console.log(itemsData)
        res.render('addShipment', {shipment: shipmentData, colors: itemsData, tgl: date, branch, branchId, token});
